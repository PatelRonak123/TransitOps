...
const getDefaultPath = (role) => {
  switch (role) {
    case "Fleet Manager": return "/vehicles";
    case "Safety Officer": return "/drivers";
    case "Financial Analyst": return "/fuel-expenses";
    case "Dispatcher":
...
const routeRoles = {
  ...
  "/settings": ["Fleet Manager", "Dispatcher", "Safety Officer", "Financial Analyst"]
};
...
function AppRoutes() {
    ...
    return (
        <Routes>
            <Route
                path="/"
                element={isAuthenticated ? <Navigate to={getDefaultPath(user?.role)} replace /> : <Home />}
            />
            <Route
                path="/dashboard"
